...
module.exports = function (client, options) {
	...
	function play(msg, suffix) {
		...
		if (msg.member.voiceChannel === undefined) return msg.channel.sendMessage('You\'re not in a voice channel.');

		// Make sure the suffix exists.
		if (!suffix) return msg.channel.sendMessage('No video specified!');
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.channel.sendMessage('Maximum queue size reached!');
		}
		...
		msg.channel.sendMessage('Searching...').then(response => {
			...
			YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				// Queue the video.
				msg.channel.sendMessage('Queued: ' + info.title).then(() => {
					queue.push(info);
	...
	function skip(msg, suffix) {
		...
		if (voiceConnection === null) return msg.channel.sendMessage('No music being played.');

		// Get the queue.
		const queue = getQueue(msg.guild.id);

		if (!canSkip(msg.member, queue)) return msg.channel.sendMessage('You cannot skip this as you didn\'t queue it.').then((response) => {
		...
		msg.channel.sendMessage('Skipped ' + toSkip + '!');
	}
	...
	function pause(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return msg.channel.sendMessage('You are not authorized to use this.');

		// Pause.
		msg.channel.sendMessage('Playback paused.');
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function leave(msg, suffix) {
		if (isAdmin(msg.member)) {
			const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
			if (voiceConnection === null) return msg.channel.sendMessage('I\'m not in any channel!.');
			// Clear the queue.
			const queue = getQueue(msg.guild.id);
	...
	function resume(msg, suffix) {
		...
		// Resume.
		msg.channel.sendMessage('Playback resumed.');
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function volume(msg, suffix) {
		...
		if (suffix > 200 || suffix < 0) return msg.channel.sendMessage('Volume out of range!').then((response) => {
			response.delete(5000);
		});

		msg.channel.sendMessage("Volume set to " + suffix);
	...
	function executeQueue(msg, queue) {
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.channel.sendMessage('Now Playing: ' + `*${video.title}*`).then(() => {
